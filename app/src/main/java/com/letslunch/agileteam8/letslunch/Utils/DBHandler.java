...
public class DBHandler
{
    ...
    public static synchronized DBHandler getInstance() {
        if(ourInstance == null) return ourInstance = new DBHandler();
    ...
    public void userResponseToEating(String userStatus) {
        ...
        databaseReference.child("GroupsAndTheirMembers")
                         ...
                         .addOnCompleteListener(activity, new OnCompleteListener<Void>()
                         {
                             ...
                             public void onComplete(@NonNull Task<Void> task) {
                                 ...
                                 if(task.isSuccessful()) {
                                     ...
                                     Toast.makeText(activity, "Your status has been updated.",
                                                    Toast.LENGTH_SHORT).show();
                                 } else {
                                     Toast.makeText(activity, "Unable to let your friends know.",
                                                    Toast.LENGTH_SHORT).show();
                                 }
                             }
                         });
    ...
    public void addCurrentUserToGroup(Group group) {
        ...
        this.databaseReference.child("GroupsAndTheirMembers")
                              .child(currentGroupID)
                              .child(currentUser.getUid())
                              .setValue(myUser)
                              .addOnCompleteListener(activity, new OnCompleteListener<Void>()
                                  ...
                                  public void onComplete(@NonNull Task<Void> task) {
                                      ...
                                      if(!task.isSuccessful()) {
                                          ...
                                          Toast.makeText(activity,
                                                         "Unable to add you to the created group. Try Manually.",
                                                         Toast.LENGTH_SHORT).show();
                                      }
                                  }
                              });
        ...
        ValueEventListener groupListener = new ValueEventListener()
        {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.hasChild(groupCode)) {
                    ...
                    Toast.makeText(activity, "Invalid Code: No group found.", Toast.LENGTH_SHORT)
                         .show();
                }
            }
            ...
            public void onCancelled(
                    DatabaseError databaseError) // Problem occurred extracting values
            {
                Toast.makeText(activity, "Unable to add you to the group. Try later.",
                               Toast.LENGTH_SHORT).show();
            }
        };
    ...
    public void addingGroupToCurrentUser(final Group createdGroup) {
        //Firebase logic for creating group
        this.databaseReference.child("UserAndTheirGroups")
                              .child(firebaseAuth.getCurrentUser().getUid())
                              .child(createdGroup.getID())
                              ...
                              .addOnCompleteListener(activity, new OnCompleteListener<Void>()
                              {
                                  ...
                                  public void onComplete(@NonNull Task<Void> task) {
                                      ...
                                      if(!task.isSuccessful()) {
                                          // Notifying the user that saving was NOT successful
                                          Toast.makeText(activity,
                                                         "Unable to add assign a group to you.",
                                                         Toast.LENGTH_SHORT).show();
                                      }
    ...
    public void saveRestaurants(String name, double latitude, double longitude, Marker m) {
        ...
        Restaurant currentRestaurant = new Restaurant(createdRestaurantID, name, latitude,
                                                      longitude);
        databaseReference.child("GroupsAndTheirRestaurants")
                         .child(currentGroupID)
                         .child(currentRestaurant.getId())
                         .setValue(currentRestaurant)
                         .addOnCompleteListener(activity, new OnCompleteListener<Void>()
                         {
                             @Override
                             public void onComplete(@NonNull Task<Void> task) {
                                 ...
                                 if(!task.isSuccessful()) {
                                     // Notifying the user that saving was NOT successful
                                     Toast.makeText(activity,
                                                    "Unable to save restaurant. Try Again",
                                                    Toast.LENGTH_SHORT).show();
                                 }
                             }
                         });
    ...
    public void joinRestaurant(String restId, String prevSelection) {
        ...
        String dispName =
                firebaseAuth.getCurrentUser().getDisplayName() == null ? "missing-disp-name"
        ...
        if(this.prevSelection == null) {
            ...
            databaseReference.child("UsersAndTheirRestaurants")
                             .child(currentGroupID)
                             .child(userId)
                             .setValue(restId);
            ...
            databaseReference.child("UsersAndTheirRestaurants")
                             .child(currentGroupID)
                             .child(getUser())
                             .addListenerForSingleValueEvent(getSelectionListener());
        } else {
